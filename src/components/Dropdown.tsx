...
interface DropdownProps {
  ...
  type: 'genre' | 'animeType' | 'producer' | 'rating' | 'source';
}
...
const Dropdown: React.FC<DropdownProps> = ({
  ...
  const renderOtherIcon = (status: string) => {
    if (status === '+') {
      return (
        <svg
          width='24'
      ...
      {isOpen && (
        ...
        >
          <div
            className={`grid ${
              type === 'genre'
                ? 'grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-36'
                : 'grid-cols-1 gap-y-2'
            }`}
            style={
              type !== 'genre'
                ? { gridAutoFlow: 'row', gridTemplateColumns: '1fr' }
                : {}
            }
          >
            {' '}
