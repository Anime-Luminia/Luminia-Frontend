...
const PlotSearch: React.FC = () => {
  ...
  const handleSearch = async () => {
    const response = await fetch(`http://localhost:8080/api/anime/search?plot=${plotQuery}`);
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen flex flex-col items-center">
      <h1 className="text-4xl font-bold text-center mb-6 text-purple-700">줄거리 찾기</h1>
      ...
      {animeList.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {animeList.map((anime: any) => (
            <div key={anime.malId} className="bg-white shadow-md rounded-lg overflow-hidden">
              <img src={anime.imageUrl} alt={anime.koreanName} className="w-full h-48 object-cover" />
              <div className="p-4">
                <h2 className="text-lg font-semibold">{anime.koreanName}</h2>
                <p className="text-sm text-gray-600">제작사: {anime.productionCompany}</p>
                <p className="text-sm text-gray-800 font-bold">평점: {anime.score}</p>
              </div>
            </div>
          ))}
        </div>
